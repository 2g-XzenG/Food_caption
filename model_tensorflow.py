...
#############################
###### Parameters #####
annotation_path = '/home/long/Desktop/show_food/feats/annotation.pickle'
feat_path = '/home/long/Desktop/show_food/feats/feats.npy'
model_path = '/home/long/Desktop/show_food/model/'
#############################
...
def test(test_feat=None, model_path='/home/long/Desktop/show_food/model/model-89', maxlen=20):
    annotation_data = pd.read_pickle(annotation_path)
    captions = annotation_data['caption'].values
    wordtoix, ixtoword, bias_init_vector = preProBuildWordVocab(captions)
    n_words = len(wordtoix)
...
def show_image(test_feat_path, test_image_path, i):
    test_feat = np.load(test_feat_path)[i]
    test_image = pd.read_pickle(test_image_path)['image'][i*150]
    test_label = pd.read_pickle(test_image_path)['caption'][i*150]

    generated_words, alpha_list = test(test_feat)
    img = crop_image(test_image)

    alphas = np.array(alpha_list).swapaxes(1,2)

    n_words = alphas.shape[0] + 1
    w = np.round(np.sqrt(n_words))-1
    h = np.ceil(np.float32(n_words) / w)-1
    ...
    for ii in xrange(alphas.shape[0]-1):
        ...
        plt.axis('off')
    plt.savefig('/home/long/Desktop/show_food/result/' + test_label + str(i) + '.jpg')
    plt.close
...
show_image(test_feat_path, test_image_path, 260)
